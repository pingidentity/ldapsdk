 ...
 * to authenticate to an UnboundID Directory Server using the
 * UNBOUNDID-YUBIKEY-OTP mechanism.  The credentials include at least an
 * authentication ID and a one-time password generated by a YubiKey device.  The
 * request may also include a static password (which may or may not be required
 * by the server) and an optional authorization ID.
 * <BR><BR>
